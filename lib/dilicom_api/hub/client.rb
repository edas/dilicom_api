  ...
  module Hub
    class Client
      ...
      def json_request(end_point, params = {}, timeout: nil)
        ...
        fail DilicomStatusError, "Dilicom returned an error status #{body['returnStatus']} in #{end_point} with #{params} : #{body['returnMessage']}" if body.key?('returnStatus') and not %w(OK WARNING).include?(body['returnStatus'])
        body
      end
